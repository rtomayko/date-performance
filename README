...
1. Implement various core Date methods in C. This is nowhere near a 
   complete rewrite of Date in C but many of the hot spots have been 
   replaced with machine code.
...
2. Provide alternate implementations of strftime and strptime in C.  The stock
   ...
   facades on top of the system's <tt>strftime(2)</tt> and
   <tt>strptime(2)</tt>. The system methods run 10x and 50x (yes, *fifty* times
   faster) than their Ruby based counterparts, respectively.  Unfortunately,
   <tt>strftime(2)</tt> and <tt>strptime(2)</tt> implementations vary from
...
This package is mostly transparent after an initial require:
    ...
    Date.new 1912, 6, 23        # Wow! That was fast!
...
The Date[http://www.ruby-doc.org/docs/rdoc/1.9/files/_/lib/date_rb.html] class
is often the cause of poor performance in Ruby programs. A frequent suggestion
is to use the Time[http://www.ruby-doc.org/docs/rdoc/1.9/classes/Time.html]
class, which is much faster, but that solution has correctness problems in a
wide range of data typing cases. It is often the case that you want separate
...
Have you read the
date.rb[http://www.ruby-doc.org/docs/rdoc/1.9/files/_/lib/date_rb.html]
documentation? The implementation is pretty hard core; it can handle a lot 
of weird cases that +Time+ does not and would appear to be a correct 
implementation of date handling, which has the usual side-effect of being slow.

The +Date+ implementation uses a single Astronomical Julian Day (AJD) number
to represent dates internally. In fact, <tt>Date#initialize</tt> takes a
